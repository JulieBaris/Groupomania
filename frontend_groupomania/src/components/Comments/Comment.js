...
class Comments extends React.Component {
    ...
    componentDidMount() {
        ...
        fetch("http://localhost:3300/api/articles" + articleId + "/comments/" ,
            {headers: 
                {"Authorization" : token},
            })
            .then(res => res.json())
        ...
        fetch("http://localhost:3300/api/profils", 
            {headers: 
                {"Authorization" : token}
            })
            .then(res => res.json())
    ...
    handleSubmit (e) {
        ...
        axios(('http://localhost:3300/api/comments/'), requestOptions)
                    ...
                    {comments.map((comment) => (
                        <div className="form-comment" key={"divimg" + comment.id}>
                            {users.map((user) => {
                                ...
                                return <img src={user.imageUrl} alt="user" key={"userImage" + comment.id} />
